...
def main():
    # warm up run
    NoiseFilteringStage(hostname=LOCAL_LLM_HOST, n_threads=10, batch_size=10).execute(["google.deepvariant.v1.6.1.CODE_COMMENT"], reverse=False)
    ...
    for threads, batches in itertools.product(n_threads, n_batches):
        start = time.time()
        NoiseFilteringStage(hostname=LOCAL_LLM_HOST, n_threads=threads, batch_size=batches).execute(["google.deepvariant"], reverse=False)
        end = time.time()
