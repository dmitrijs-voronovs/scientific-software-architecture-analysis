...
def split_file_in_sequential_batches(path: Path, handler: IDfHandler, batch_size: int = 3000):
    df = handler.read_df(path)
    path.rename(path.with_stem(f"_{path.stem}"))
    ...
    for i in range(n_parts):
        start = i * batch_size
        part_name = path if i == 0 else path.with_stem(f"{path.stem}.part_{i+1:0{len(str(n_parts))}}")
...
def main():
    ...
    split_file_in_sequential_batches(Path(r"C:\Users\Dmitrijs\Documents\myDocs\masters\courses\thesis\code\data\keywords_2\s2_arch_relevance_check_o\allenai.scispacy.v0.5.5.issue.parquet"), ParquetDFHandler(), 100)
